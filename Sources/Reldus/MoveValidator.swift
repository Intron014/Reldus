...
class MoveValidator {
  ...
  static func isKingInCheck(board: ChessBoard, color: Color) -> Bool {
    ...
    if let kingSquare = kingSquare as Int? {
  ...
  private func validatePawnMove(move: Move, board: ChessBoard, piece: ChessPiece) -> Bool {
    ...
    if move.to == move.from + direction * 16 && board.pieceAt(square: move.to) == nil
      && board.pieceAt(square: move.from + direction * 8) == nil && move.from / 8 == startRank
    {
      return true
    }

    if abs(toFile - fromFile) == 1 && move.to == move.from + direction * 8
      && board.pieceAt(square: move.to) != nil
    {
      return true
    }

    if abs(toFile - fromFile) == 1 && move.to == move.from + direction * 8
      && board.enPassantSquare == move.to
    {
      return true
    }
  ...
  private func validateQueenMove(move: Move, board: ChessBoard) -> Bool {
    return validateBishopMove(move: move, board: board)
      || validateRookMove(move: move, board: board)
  }
  ...
  private func validateKingMove(move: Move, board: ChessBoard) -> Bool {
    ...
    if fileDiff == 2 && rankDiff == 0 {
      ...
      if let rook = board.pieceAt(square: rookSquare),
        rook == (board.turn == .white ? .whiteRook : .blackRook)
      {
        let kingSide = toFile == 6
        let pathSquares =
          kingSide ? [fromFile + 1, fromFile + 2] : [fromFile - 1, fromFile - 2, fromFile - 3]
