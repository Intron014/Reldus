...
class UCI {
  ...
  private func parseMove(moveString: String) -> Move? {
    ...
    let fromRank = Int(
      moveString[moveString.index(moveString.startIndex, offsetBy: 1)].asciiValue! - Character("1")
        .asciiValue!)
    let toFile = Int(
      moveString[moveString.index(moveString.startIndex, offsetBy: 2)].asciiValue! - Character("a")
        .asciiValue!)
    let toRank = Int(
      moveString[moveString.index(moveString.startIndex, offsetBy: 3)].asciiValue! - Character("1")
        .asciiValue!)
    ...
    let move = Move(
      from: from, to: to, piece: piece, capturedPiece: capturedPiece, promotion: promotion,
      isEnPassant: isEnPassant, isCastling: isCastling, isCheck: false)
