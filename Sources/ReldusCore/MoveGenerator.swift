...
class MoveGenerator {
  ...
  static func generateMoves(for board: ChessBoard, color: Color) -> [Move] {
    ...
    moves.append(
      contentsOf: generateSlidingPieceMoves(
        for: board, color: color, pieceTypes: slidingPieces(for: color)))
    ...
    for square in 0..<64 {
      if pawnBitboard.isBitSet(at: square) {
        ...
        if !board.getOccupancy().isBitSet(at: toSquare) {
          let mmove = Move(
            from: square, to: toSquare, piece: color == .white ? .whitePawn : .blackPawn)
          ...
          if (color == .white && rank == 1) || (color == .black && rank == 6) {
            ...
            if !board.getOccupancy().isBitSet(at: doubleSquare) {
              let mmove = Move(
                from: square, to: doubleSquare, piece: color == .white ? .whitePawn : .blackPawn)
              // if MoveValidator().isMoveLegal(move: mmove, board: board) {
              //     moves.append(mmove)
              // }
              moves.append(mmove)
        ...
        if file > 0 {
          ...
          if board.getOccupancy().isBitSet(at: captureSquare),
            let capturedPiece = board.pieceAt(square: captureSquare),
            capturedPiece.character.isUppercase != (color == .white)
          {
            let mmove = Move(
              from: square, to: captureSquare, piece: color == .white ? .whitePawn : .blackPawn,
      ...
      if knightBitboard.isBitSet(at: square) {
        ...
        for (df, dr) in knightMoves {
          ...
          if newFile >= 0, newFile < 8, newRank >= 0, newRank < 8 {
            ...
            if !board.getOccupancy().isBitSet(at: toSquare) {
              let mmove = Move(
                from: square, to: toSquare, piece: color == .white ? .whiteKnight : .blackKnight)
              // if MoveValidator().isMoveLegal(move: mmove, board: board) {
              //     moves.append(mmove)
              // }
              moves.append(mmove)
            } else if let capturedPiece = board.pieceAt(square: toSquare),
              capturedPiece.character.isUppercase != (color == .white)
            {
              let mmove = Move(
                from: square, to: toSquare, piece: color == .white ? .whiteKnight : .blackKnight,
  ...
  private static func generateSlidingPieceMoves(
    for board: ChessBoard, color: Color, pieceTypes: [ChessPiece]
  ) -> [Move] {
    var moves: [Move] = []
          ...
          if let pieceDirections = directions[pieceType] {
            for (df, dr) in pieceDirections {
              ...
              while newFile >= 0, newFile < 8, newRank >= 0, newRank < 8 {
                ...
                {
                  let mmove = Move(
                    from: square, to: toSquare, piece: pieceType, capturedPiece: capturedPiece)
                  // if MoveValidator().isMoveLegal(move: mmove, board: board) {
                  //     moves.append(mmove)
                  // }
                  moves.append(mmove)
      ...
      if kingBitboard.isBitSet(at: square) {
        ...
        for (df, dr) in kingMoves {
          ...
          if newFile >= 0, newFile < 8, newRank >= 0, newRank < 8 {
            ...
            if !board.getOccupancy().isBitSet(at: toSquare) {
              let mmove: Move = Move(
                from: square, to: toSquare, piece: color == .white ? .whiteKing : .blackKing)
              // if MoveValidator().isMoveLegal(move: mmove, board: board) {
              //     moves.append(mmove)
              // }
              moves.append(mmove)
  ...
  private static func slidingPieces(for color: Color) -> [ChessPiece] {
    return color == .white
      ? [.whiteBishop, .whiteRook, .whiteQueen] : [.blackBishop, .blackRook, .blackQueen]
  }
}
